...
public class Utils {
	...
	public static void broadcastMessageToServer(String message, String exemptPlayer) {
		if (plugin.BROADCAST_TO_SERVER) {
			if (plugin.ONLY_BROADCAST_ONLINE && !Utils
			        .playerIsOnline(exemptPlayer))
				return;
	...
	private static String getAwardPrizesString(Award award) {
		...
		if (award.hasDescription()) {
			...
			if (award.hasXpLevels()) {
				...
				sb.append(award.getXpLevels() + " " + plugin.XPLEVELS_DEF
				        .toLowerCase());
			}
			if (award.hasItems()) {
	...
	public static String getItemListSentance(ItemStack[] items) {
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (itemName.equals("potion")) {
				...
				itemName += " of " + potion.getType().toString().toLowerCase()
				        .replace("_", " ");
			...
			if (is.getItemMeta().hasEnchants() || (useStorage && esm
			        .hasStoredEnchants())) {
				sb.append("(with ");
				...
				for (Enchantment enchant : enchantKeys) {
					...
					sb.append(getNameFromEnchant(enchant) + " " + Integer
					        .toString(level) + ", ");
				}
				sb.delete(sb.length() - 2, sb.length());
				sb.append(")");
	...
	public static String getKnownWebsites() {
		ConfigAccessor websiteFile = new ConfigAccessor(
		        "data" + File.separator + "known websites.yml");
	...
	public static String helpMenu(CommandSender player) {
		...
		if (player.hasPermission("voteroulette.viewrewards")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.REWARDS_PURAL_DEF
			        .toLowerCase() + ChatColor.GRAY + " - See rewards you are eligible to get.\n");
		}
		if (player.hasPermission("voteroulette.viewmilestones")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.MILESTONE_PURAL_DEF
			        .toLowerCase() + ChatColor.GRAY + " - See milestones you are eligible to get.\n");
		}
		if (player.hasPermission("voteroulette.viewallawards")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " [" + plugin.REWARDS_PURAL_DEF
			        .toLowerCase() + "/" + plugin.MILESTONE_PURAL_DEF
			        .toLowerCase() + "]" + " -a" + ChatColor.GRAY + " - See all the awards, regardless of if you are eligable.\n");
		}
		if (player.hasPermission("voteroulette.top10")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.TOP_DEF + "10 " + plugin.TOTAL_DEF
			        .toLowerCase().replace(" ", "") + ChatColor.GRAY + " - See the top 10 players for total votes.\n");
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.TOP_DEF + "10 " + plugin.VOTE_STREAK_DEF
			        .toLowerCase().replace(" ", "") + ChatColor.GRAY + " - See the top 10 players for consecutive days voting.\n");
		}
		if (player.hasPermission("voteroulette.lastvote")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.LASTVOTE_DEF
			        .toLowerCase() + ChatColor.GRAY + " - Shows how long ago your last vote was.\n");
		}
		if (player.hasPermission("voteroulette.lastvoteothers")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.LASTVOTE_DEF
			        .toLowerCase() + " [" + plugin.PLAYER_DEF.toLowerCase() + "]" + ChatColor.GRAY + " - Shows how long ago the given players last vote was.\n");
		}
		if (player.hasPermission("voteroulette.viewstats")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.STATS_DEF
			        .toLowerCase() + ChatColor.GRAY + " - See your voting stats.\n");
		}
		if (player.hasPermission("voteroulette.viewotherstats")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.STATS_DEF
			        .toLowerCase() + " [" + plugin.PLAYER_DEF.toLowerCase() + "]" + ChatColor.GRAY + " - See the stats of another player.\n");
		}
		if (player.hasPermission("voteroulette.editstats")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.STATS_DEF
			        .toLowerCase() + " [" + plugin.PLAYER_DEF.toLowerCase() + "] " + plugin.SETTOTAL_DEF + " [#]" + ChatColor.GRAY + " - Set a players total votes.\n");
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.STATS_DEF
			        .toLowerCase() + " [" + plugin.PLAYER_DEF.toLowerCase() + "] " + plugin.SETCYCLE_DEF + " [#]" + ChatColor.GRAY + " - Set a players current vote cycle.\n");
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.STATS_DEF
			        .toLowerCase() + " [" + plugin.PLAYER_DEF.toLowerCase() + "] " + plugin.SETSTREAK_DEF + " [#]" + ChatColor.GRAY + " - Set a players current vote streak.\n");
		}
		...
		sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.CLAIM_DEF + " " + plugin.REWARDS_PURAL_DEF
		        .toLowerCase() + ChatColor.GRAY + " - Lists any of your unclaimed rewards.\n");
		sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.CLAIM_DEF + " " + plugin.REWARDS_PURAL_DEF
		        .toLowerCase() + " [#/" + plugin.ALL_DEF + "]" + ChatColor.GRAY + " - Gives you the reward with the given # or all of them.\n");
		sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.CLAIM_DEF + " " + plugin.MILESTONE_PURAL_DEF
		        .toLowerCase() + ChatColor.GRAY + " - Lists any of your unclaimed milestones.\n");
		...
		if (player.hasPermission("voteroulette.forceawards")) {
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.FORCEREWARD_DEF + "  " + "[reward#] " + "[" + plugin.PLAYER_DEF
			        .toLowerCase() + "]" + ChatColor.GRAY + " - Award a player the given reward. The number corresponds with the full rewards list.\n");
			sb.append(ChatColor.AQUA + "/" + plugin.DEFAULT_ALIAS + " " + plugin.FORCEMILESTONE_DEF + "  " + "[milestone#] " + "[" + plugin.PLAYER_DEF
			        .toLowerCase() + "]" + ChatColor.GRAY + " - Award a player the given milestone. The number corresponds with the full milestones list.\n");
		}
	...
	public static void showAwardGUI(Award award, Player p, int awardNumber, boolean showPlayers) {
		...
		if (((award.getAwardType() == AwardType.REWARD) && (reward
		        .hasWebsites() || reward.hasVoteStreak())) || award.hasChance() || award
		        .hasWorlds() || award.hasReroll() || award.hasDescription() || ((plugin.SHOW_COMMANDS_IN_AWARD || showPlayers) && (award
		        .hasPlayers() || award.hasPermissionGroups())) || (award
		        .getAwardType() == AwardType.MILESTONE)) {
			req++;
		}
		while (req > multOf9) {
			multOf9 += 9;
		...
		if (award.hasXpLevels()) {
			...
			itemMeta.setDisplayName(ChatColor.YELLOW + Integer.toString(award
			        .getXpLevels()) + ChatColor.RESET + " " + plugin.XPLEVELS_DEF);
		...
		if (award.hasCurrency()) {
			...
			ItemMeta itemMeta = xp.getItemMeta();
			itemMeta.setDisplayName(ChatColor.YELLOW + Double.toString(award
			        .getCurrency()) + ChatColor.RESET + " " + plugin.CURRENCY_PURAL_DEF);
			xp.setItemMeta(itemMeta);
			i.addItem(xp);
		}
		if (award.hasCommands()) {
		...
		if (((award.getAwardType() == AwardType.REWARD) && (reward
			...
			if ((award.getAwardType() == AwardType.REWARD) && reward
			        .hasVoteStreak()) {
				StringBuilder sb = new StringBuilder();
				sb.append(ChatColor.GOLD + plugin.VOTE_STREAK_DEF + ": " + ChatColor.DARK_AQUA + reward
				        .getVoteStreak());
				if (reward.hasVoteStreakModifier()) {
					VoteStreakModifier vsm = reward.getVoteStreakModifier();
					sb.append(" " + vsm.toString().toLowerCase()
					        .replace("_", " "));
				}
				sb.append(" ");
			...
			if (award.hasDescription()) {
				StringBuilder sb = new StringBuilder();
				sb.append(ChatColor.GOLD + "Description: " + ChatColor.DARK_AQUA + award
				        .getDescription());
	...
	public static void showTopInChat(Player player, StatType stat) {
		...
		if (topStats != null) {
			...
			for (VoterStat vs : topStats) {
				...
				message += ChatColor.GOLD + statCount + " " + ChatColor.WHITE + vs
				        .getPlayerName();
	...
	public static void showTopScoreboard(final Player player, StatType stat) {
		...
		if (stat == StatType.LONGEST_VOTE_STREAKS) {
			objective
			        .setDisplayName(ChatColor.AQUA + plugin.LONGEST_VOTE_STREAK_DEF);
			List<VoterStat> topStats = VoteRoulette.getStatsManager()
			        .getTopLongestVotestreaks();
			if (topStats == null) {
				player.sendMessage(ChatColor.RED + "Error: stats are empty.");
