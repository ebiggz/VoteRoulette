			...
			if (items != null) {
				for (String itemID : items.getKeys(false)) {
					...
					ConfigurationSection itemData = items
					        .getConfigurationSection(itemID);
					// ItemStack item;
					// ItemMeta itemMeta;
					// parseConfigItemData
					if (itemData != null) {
						if (itemData.contains("multiple")) {
							ConfigurationSection multipleData = itemData
							        .getConfigurationSection("multiple");
							for (String mItemID : multipleData.getKeys(false)) {
								ItemStack mItem = parseConfigItemData(
								        id,
								        multipleData
								                .getConfigurationSection(mItemID));
								if (mItem != null) {
		...
		if (itemData != null) {
			...
			if (itemData.contains("enchants")) {
				...
				for (String enchantName : tmp) {
					...
					try {
						Enchantment enchant = Utils
						        .getEnchantEnumFromName(enchantName);
