...
public class VoteProcessor implements Runnable {
	...
	private void giveRandomReward(String playerName, Reward[] qualRewards) {
		...
		if (rm.awardsContainChance(qualRewards)) {
			...
			for (Reward reward : rewardsWithChance) {
				Utils.debugMessage("Checking \"" + reward.getName() + "\" at " + reward
				        .getChanceMin() + " in " + reward.getChanceMax());
				int random = 1 + (int) (Math.random() * ((reward.getChanceMax() - 1) + 1));
				if (random > reward.getChanceMin()) {
					Utils.debugMessage("Failed (" + random + ").");
					continue;
				}
				Utils.debugMessage("Passed (" + random + "). Administering \"" + reward
				        .getName() + "\" to " + playerName);
	...
	public void run() {
		if (!voter.isReal()) {
			plugin.getLogger()
			        .warning(
			                "A vote was recieved from the username \"" + playerName + "\" but VoteRoulette could not find a UUID for this name! Rewards will not be given and stats will not update. Maybe the name was typed incorrectly or there is a connectivity issue with Mojang's UUID server?");
			return;
		}
		...
		if (!ignoreBlackList) {
			if (((plugin.BLACKLIST_AS_WHITELIST == false) && Utils
			        .playerIsBlacklisted(playerName)) || (plugin.BLACKLIST_AS_WHITELIST && (Utils
