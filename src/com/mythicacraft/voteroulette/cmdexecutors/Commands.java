...
public class Commands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (commandLabel.equalsIgnoreCase("vote") || commandLabel.equalsIgnoreCase("votelinks") || commandLabel.equalsIgnoreCase("votesites") || commandLabel.equalsIgnoreCase(plugin.VOTE_DEF.toLowerCase())) {
			...
			if (args.length == 0) {
					...
					if (sender.hasPermission("voteroulette.deleteawards")) {
						if (args.length >= 2) {
							...
							try {
								...
								sender.sendMessage(ChatColor.AQUA + "[VoteRoulette] You successfully deleted the Reward " + ChatColor.YELLOW + reward.getName() + ChatColor.YELLOW + "!");
							} catch (Exception e) {
								sender.sendMessage(plugin.INVALID_NUMBER_NOTIFICATION);
							}
						} else {
				...
				if (args[0].equalsIgnoreCase("deletemilestone")) {
					if (sender.hasPermission("voteroulette.deleteawards")) {
						if (args.length >= 2) {
							...
							try {
								...
								sender.sendMessage(ChatColor.AQUA + "[VoteRoulette] You successfully deleted the Milestone " + ChatColor.YELLOW + milestone.getName() + ChatColor.YELLOW + "!");
							} catch (Exception e) {
								sender.sendMessage(plugin.INVALID_NUMBER_NOTIFICATION);
							}
						} else {
					...
					return true;
				} else if (args[0].equalsIgnoreCase(plugin.TOP_DEF + "10") || args[0].equalsIgnoreCase(plugin.TOP_DEF)) {
					...
					if (args.length == 1) {
						...
						if (plugin.isOn1dot7) {
							...
							InteractiveMessage totalVotes = new InteractiveMessage(new InteractiveMessageElement(new FormattedText("- " + plugin.TOTAL_VOTES_DEF, ChatColor.YELLOW), HoverEvent.SHOW_TEXT, new FormattedText("Click for stats"), ClickEvent.RUN_COMMAND, "/" + plugin.DEFAULT_ALIAS + " " + plugin.TOP_DEF + " " + plugin.TOTAL_DEF.toLowerCase().replace(" ", "")));
							totalVotes.sendTo(sender);
							if (VoteRoulette.USE_DATABASE) {
								InteractiveMessage currentMonth = new InteractiveMessage(new InteractiveMessageElement(new FormattedText("- " + plugin.CURRENT_MONTHS_VOTES_DEF, ChatColor.YELLOW), HoverEvent.SHOW_TEXT, new FormattedText("Click for stats"), ClickEvent.RUN_COMMAND, "/" + plugin.DEFAULT_ALIAS + " " + plugin.TOP_DEF + " " + plugin.CURRENT_MONTH_DEF));
								currentMonth.sendTo(sender);
								InteractiveMessage previousMonth = new InteractiveMessage(new InteractiveMessageElement(new FormattedText("- " + plugin.PREVIOUS_MONTHS_VOTES_DEF, ChatColor.YELLOW), HoverEvent.SHOW_TEXT, new FormattedText("Click for stats"), ClickEvent.RUN_COMMAND, "/" + plugin.DEFAULT_ALIAS + " " + plugin.TOP_DEF + " " + plugin.PREVIOUS_MONTH_DEF));
								previousMonth.sendTo(sender);
							}
							InteractiveMessage longestVotestreaks = new InteractiveMessage(new InteractiveMessageElement(new FormattedText("- " + plugin.LONGEST_VOTE_STREAK_DEF, ChatColor.YELLOW), HoverEvent.SHOW_TEXT, new FormattedText("Click for stats"), ClickEvent.RUN_COMMAND, "/" + plugin.DEFAULT_ALIAS + " " + plugin.TOP_DEF + " " + plugin.LONGEST_STREAK_DEF));
					...
					if (args.length == 1) {
						...
						if (voter.hasLastVoteTimeStamp()) {
							String timeSince = Utils.getTimeSinceString(voter.getLastVoteTimeStamp());
							sender.sendMessage(plugin.LAST_VOTE_SELF_CMD.replace("%time%", timeSince));
						} else {
					...
					if (args.length == 1) {
						sender.sendMessage(ChatColor.RED + "[VoteRoulette] /" + plugin.DEFAULT_ALIAS + " " + plugin.FORCEVOTE_DEF + " [" + plugin.PLAYER_DEF.toLowerCase() + "]");
						return true;
					}
					if (args.length > 1) {
				...
				else if (args[0].equalsIgnoreCase(plugin.FORCEREWARD_DEF.toLowerCase())) {
					if (!sender.hasPermission("voteroulette.forceawards")) {
						sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
						return true;
					}
					if (args.length < 3) {
						sender.sendMessage(ChatColor.RED + "[VoteRoulette] /" + plugin.DEFAULT_ALIAS + " " + plugin.FORCEREWARD_DEF + " " + "[reward#] " + "[" + plugin.PLAYER_DEF.toLowerCase() + "]");
						return true;
					}
					if (args.length >= 3) {
						String otherPlayer = Utils.completeName(args[2]);
						if (otherPlayer == null) {
							sender.sendMessage(plugin.CANT_FIND_PLAYER_NOTIFICATION.replace("%player%", args[2]));
							return true;
						}
						int rewardIndex;
						try {
					...
					if (args.length < 3) {
						sender.sendMessage(ChatColor.RED + "[VoteRoulette] /" + plugin.DEFAULT_ALIAS + " " + plugin.FORCEMILESTONE_DEF + " " + "[milestone#] " + "[" + plugin.PLAYER_DEF.toLowerCase() + "]");
						return true;
					}
					if (args.length >= 3) {
						String otherPlayer = Utils.completeName(args[2]);
				...
				else if (args[0].equalsIgnoreCase(plugin.REMIND_DEF) || args[0].equalsIgnoreCase("reminder") || args[0].equalsIgnoreCase("broadcast")) {
					...
					if (args.length == 1) {
						...
						sender.sendMessage(ChatColor.GOLD + "-----" + ChatColor.GREEN + plugin.VOTE_DEF + " " + plugin.STATS_DEF + ": " + ChatColor.YELLOW + sender.getName() + ChatColor.GOLD + "-----"); // header
																																																			// of
																																																			// page
																																																			// with
																																																			// current
						...
						} else if (args[2].equalsIgnoreCase(plugin.SETCYCLE_DEF)) {
						...
						} else if (args[2].equalsIgnoreCase(plugin.SETSTREAK_DEF)) {
				...
				// send vote totals, how many votes till next milestone, how
				// many votes in threshold
					...
					if (!plugin.GUI_FOR_AWARDS || !(sender instanceof Player)) {
						for (int i = 0; i < milestones.length; i++) {
							message += ChatColor.YELLOW + plugin.MILESTONE_DEF + " #" + Integer.toString(i + 1) + ": " + ChatColor.AQUA + milestones[i].getName() + "\n";
							message += ChatColor.GOLD + plugin.VOTES_DEF + ": " + ChatColor.DARK_AQUA + milestones[i].getVotes() + "\n";
							...
							if (milestones[i].hasChance()) {
								message += ChatColor.GOLD + plugin.CHANCE_DEF + ": " + ChatColor.DARK_AQUA + milestones[i].getChanceMin() + " in " + milestones[i].getChanceMax() + "\n";
							}
							if (milestones[i].hasReroll()) {
								message += ChatColor.GOLD + "Reroll For: " + ChatColor.DARK_AQUA + milestones[i].getRerollString().replace("ANY", "Random Check Single Reward").replace("RANDOM", "Random Check Single Reward").replace("ALL", "Random Check All Rewards") + "\n";
							}
						}
					} else {
						if (plugin.isOn1dot7 && args.length == 1) {
							showClickableAwardList(sender, AwardType.MILESTONE, milestones, 1, seeingAll);
							return true;
						}
						...
						message = message + ChatColor.GOLD + "Type " + ChatColor.YELLOW + "/" + plugin.DEFAULT_ALIAS + " " + plugin.MILESTONE_PURAL_DEF.toLowerCase() + " see [#]" + ChatColor.GOLD + " to see details.\n";
						for (int i = 0; i < milestones.length; i++) {
							message += ChatColor.YELLOW + Integer.toString(i + 1) + ") " + ChatColor.AQUA + milestones[i].getName() + "\n";
						}
					}
					...
					if (args.length >= 2) {
						if (plugin.GUI_FOR_AWARDS && sender instanceof Player) {
							if (args[1].equalsIgnoreCase("see")) {
								if (args.length < 3) {
									sender.sendMessage(ChatColor.RED + "[VoteRoulette] /" + plugin.DEFAULT_ALIAS + " " + plugin.MILESTONE_PURAL_DEF.toLowerCase() + " see [#]");
								} else {
									try {
										int milestoneNumber = Integer.parseInt(args[2]);
										if (args.length == 4) {
											if (args[3].equalsIgnoreCase("-a")) {
												ArrayList<Milestone> milestonesAL = rm.getMilestones();
												milestones = new Milestone[milestonesAL.size()];
												milestonesAL.toArray(milestones);
											}
										}
					...
					if (!plugin.GUI_FOR_AWARDS || !(sender instanceof Player)) {
						for (int i = 0; i < rewards.length; i++) {
							if (rewards[i].isEmpty())
								continue;
							message = message + ChatColor.YELLOW + plugin.RELOAD_DEF + " #" + Integer.toString(i + 1) + ": " + ChatColor.AQUA + rewards[i].getName() + "\n";
							...
							if (rewards[i].hasVoteStreak()) {
								message += ChatColor.GOLD + plugin.VOTE_STREAK_DEF + ": " + ChatColor.DARK_AQUA + Integer.toString(rewards[i].getVoteStreak());
								if (rewards[i].hasVoteStreakModifier()) {
									VoteStreakModifier vsm = rewards[i].getVoteStreakModifier();
									message += " " + vsm.toString().toLowerCase().replace("_", " ") + " ";
							...
							if (rewards[i].hasChance()) {
								message += ChatColor.GOLD + plugin.CHANCE_DEF + ": " + ChatColor.DARK_AQUA + rewards[i].getChanceMin() + " in " + rewards[i].getChanceMax() + "\n";
							}
							if (rewards[i].hasReroll()) {
								message += ChatColor.GOLD + "Reroll For: " + ChatColor.DARK_AQUA + rewards[i].getRerollString().replace("ANY", "Random Check Single Reward").replace("RANDOM", "Random Check Single Reward").replace("ALL", "Random Check All Rewards") + "\n";
							}
						}
					} else {
						if (plugin.isOn1dot7 && args.length == 1) {
							showClickableAwardList(sender, AwardType.REWARD, rewards, 1, seeingAll);
							return true;
						}
						if (plugin.isOn1dot7 && args.length == 2) {
							if (args[1].equalsIgnoreCase("-a")) {
						...
						message = message + ChatColor.GOLD + "Type " + ChatColor.YELLOW + "/" + plugin.DEFAULT_ALIAS + " " + plugin.REWARDS_PURAL_DEF.toLowerCase() + " see [#]" + ChatColor.GOLD + " to see details.\n";
						for (int i = 0; i < rewards.length; i++) {
							if (rewards[i].isEmpty())
								continue;
							message = message + ChatColor.YELLOW + Integer.toString(i + 1) + ") " + ChatColor.AQUA + rewards[i].getName() + "\n";
						}
					}

					Paginate rewardPag = new Paginate(message, plugin.REWARDS_PURAL_DEF, commandLabel + " " + plugin.REWARDS_PURAL_DEF);

					if (args.length >= 2) {
						if (plugin.GUI_FOR_AWARDS && sender instanceof Player) {
							if (args[1].equalsIgnoreCase("see")) {
								if (args.length < 3) {
									sender.sendMessage(ChatColor.RED + "[VoteRoulette] /" + plugin.DEFAULT_ALIAS + " " + plugin.REWARDS_PURAL_DEF.toLowerCase() + " see [#]");
								} else {
									try {
										int rewardNumber = Integer.parseInt(args[2]);
										if (args.length == 4) {
											if (args[3].equalsIgnoreCase("-a")) {
												ArrayList<Reward> rewardsAL = rm.getRewards();
												rewards = new Reward[rewardsAL.size()];
						...
						try {
							...
							message = message + ChatColor.GOLD + "Type " + ChatColor.YELLOW + "/" + plugin.DEFAULT_ALIAS + " " + plugin.REWARDS_PURAL_DEF.toLowerCase() + " see [#]";
							if (seeingAll) {
								message += " -a";
							}
					...
					if (args.length == 1) {
						...
						if (args[1].equalsIgnoreCase(plugin.REWARDS_PURAL_DEF)) {
							...
							if (args.length == 2) {
								...
								if (plugin.isOn1dot7) {
									...
									all.addElement(new InteractiveMessageElement(new FormattedText("claim all", ChatColor.AQUA), HoverEvent.SHOW_TEXT, new FormattedText("Click me to claim all rewards!"), ClickEvent.RUN_COMMAND, "/" + plugin.DEFAULT_ALIAS + " " + plugin.CLAIM_DEF.toLowerCase() + " " + plugin.REWARDS_PURAL_DEF.toLowerCase() + " all"));
									...
									for (Reward reward : unclaimedRewards) {
										...
										im.addElement(new InteractiveMessageElement(new FormattedText(reward.getName(), ChatColor.AQUA), HoverEvent.SHOW_TEXT, new FormattedText("Required Slots: " + reward.getRequiredSlots(voter)), ClickEvent.RUN_COMMAND, "/" + plugin.DEFAULT_ALIAS + " " + plugin.CLAIM_DEF.toLowerCase() + " " + plugin.REWARDS_PURAL_DEF.toLowerCase() + " " + count));
										im.sendTo(sender);
										count++;
									}
								} else {
						...
						} else if (args[1].equalsIgnoreCase(plugin.MILESTONE_PURAL_DEF)) {
							...
							if (args.length == 2) {
								if (plugin.isOn1dot7) {
									...
									all.addElement(new InteractiveMessageElement(new FormattedText("claim all", ChatColor.AQUA), HoverEvent.SHOW_TEXT, new FormattedText("Click me to claim all milestones!"), ClickEvent.RUN_COMMAND, "/" + plugin.DEFAULT_ALIAS + " " + plugin.CLAIM_DEF.toLowerCase() + " " + plugin.MILESTONE_PURAL_DEF.toLowerCase() + " all"));
									all.addElement(")", ChatColor.GRAY);
									all.sendTo(sender);
									for (Milestone milestone : unclaimedMilestones) {
										InteractiveMessage im = new InteractiveMessage();
										im.addElement(count + ") ", ChatColor.YELLOW);
										im.addElement(new InteractiveMessageElement(new FormattedText(milestone.getName(), ChatColor.AQUA), HoverEvent.SHOW_TEXT, new FormattedText("Required Slots: " + milestone.getRequiredSlots(voter)), ClickEvent.RUN_COMMAND, "/" + plugin.DEFAULT_ALIAS + " " + plugin.CLAIM_DEF.toLowerCase() + " " + plugin.MILESTONE_PURAL_DEF.toLowerCase() + " " + count));
										im.sendTo(sender);
										count++;
									}
								} else {
	...
	void showClickableAwardList(CommandSender sender, AwardType type,
			Award[] awards, int pageNumber, boolean seeingAll) {
		...
		while (count < 7 * pageNumber && count < awards.length) {
			...
			String command = "/" + plugin.DEFAULT_ALIAS + " " + typeStr.toLowerCase() + " see " + Integer.toString(count + 1);
			if (seeingAll) {
				command += " -a";
			}
			tellRaw = tellRaw.replace("%player%", sender.getName()).replace("%num%", Integer.toString(count + 1)).replace("%name%", awards[count].getName()).replace("%command%", command);
			Bukkit.getServer().dispatchCommand(Bukkit.getServer().getConsoleSender(), tellRaw);
			count++;
		}
		if (pageNumber < totalPages) {
			sender.sendMessage(ChatColor.GOLD + "Type " + "\"/" + plugin.DEFAULT_ALIAS + " " + typeStr.toLowerCase() + " " + Integer.toString(pageNumber + 1) + "\" for the next page.");
		}
	}
}
