...
public class Commands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (commandLabel.equalsIgnoreCase("vote")
		        || commandLabel.equalsIgnoreCase("votelinks")
		        || commandLabel.equalsIgnoreCase("votesites")
		        || commandLabel.equalsIgnoreCase(plugin.VOTE_DEF.toLowerCase())) {
					...
					if (plugin.isOn1dot7) {
						for (String line : plugin.VOTE_WEBSITES) {
							...
							if (!fancyLink.isEmpty()) {
								String tellRaw = "tellraw %player% {\"text\":\"\",\"extra\":[{\"text\":\"%before%\",\"color\":\"white\"},{\"text\":\"%site%\",\"color\":\"white\",\"clickEvent\":{\"action\":\"open_url\",\"value\":\"%link%\"},\"hoverEvent\":{\"action\":\"show_text\",\"value\":\""
								        + ChatColor.YELLOW
								        + Utils.transcribeColorCodes(plugin.FANCY_LINK_POPUP)
			...
			if (commandLabel.equalsIgnoreCase("vr")
			        || commandLabel.equalsIgnoreCase("voteroulette")
			        || commandLabel.equalsIgnoreCase("vtr")) {
				if (args.length == 0) {
					...
					if (args.length >= 1) {
						if (args[0].equalsIgnoreCase("setname")) {
								...
								if (itemInHand != null
									...
									if (args.length == 1) {
										...
										sender.sendMessage(ChatColor.AQUA
										        + "[VoteRoulette] You have cleared the custom name of this "
										        + ChatColor.YELLOW
										        + itemInHand.getType()
										                .toString()
										                .toLowerCase()
										...
										sender.sendMessage(ChatColor.AQUA
										        + "[VoteRoulette] You have set the custom name of this "
										        + ChatColor.YELLOW
										        + itemInHand.getType()
										                .toString()
										                .toLowerCase()
									...
									sender.sendMessage(ChatColor.RED
									        + "[VoteRoulette] You must be holding an item to set it's name!");
								}
							} else {
								sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
							}
							...
							if (args[0].equalsIgnoreCase("setlore")) {
								if (sender
								        .hasPermission("voteroulette.edititems")) {
									Player p = (Player) sender;
									ItemStack itemInHand = p.getItemInHand();
									...
									if (itemInHand != null
										...
										if (args.length == 1) {
											im.setLore(null);
											sender.sendMessage(ChatColor.AQUA
											        + "[VoteRoulette] You have cleared the custom lore of this "
											        + ChatColor.YELLOW
											        + itemInHand.getType()
											                .toString()
											                .toLowerCase()
											...
											sb.delete(sb.length() - 1,
											        sb.length());
											// set name
											String loreStr = Utils
											        .transcribeColorCodes(sb
											                .toString());
											String[] loreArray = loreStr
											        .split("/");
											im.setLore(Arrays.asList(loreArray));
											sender.sendMessage(ChatColor.AQUA
											        + "[VoteRoulette] You have set the custom lore of this "
											        + ChatColor.YELLOW
											        + itemInHand.getType()
											                .toString()
											                .toLowerCase()
										...
										sender.sendMessage(ChatColor.RED
										        + "[VoteRoulette] You must be holding an item to set it's lore!");
									}
								} else {
									sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
								}
								...
								if (args[0].equalsIgnoreCase("colors")) {
									if (sender
									        .hasPermission("voteroulette.colors")) {
										sender.sendMessage(ChatColor.WHITE
										        + "Colors:" + ChatColor.BLACK
										        + " &0" + ChatColor.DARK_BLUE
										        + " &1" + ChatColor.DARK_GREEN
										        + " &2" + ChatColor.DARK_AQUA
										        + " &3" + ChatColor.DARK_RED
										        + " &4" + ChatColor.DARK_PURPLE
										        + " &5" + ChatColor.GOLD
										        + " &6" + ChatColor.GRAY
										        + " &7" + ChatColor.DARK_GRAY
										        + " &8" + ChatColor.BLUE
										        + " &9" + ChatColor.GREEN
										        + " &a" + ChatColor.AQUA
										        + " &b" + ChatColor.RED + " &c"
									...
									if (args[0].equalsIgnoreCase("create")) {
										if (!(sender instanceof Player)) {
											sender.sendMessage(ChatColor.RED
											        + "[VoteRoulette] You must be in-game to run this command!");
											return true;
										}
										if (sender
										        .hasPermission("voteroulette.createawards")) {
											...
											if (VoteRoulette.inAwardCreator
												...
												if (ac.isPaused()) {
													sender.sendMessage(ChatColor.YELLOW
													        + "[VoteRoulette] "
													        + ChatColor.AQUA
													        + "Entered the Award Creator!");
													ac.setPaused(false);
													sender.sendMessage(ChatColor.GRAY
													        + "(You have resumed the creation of a previous award)");
													ac.goToStage(ac
													        .getCurrentStage(),
													        true);
												} else {
													sender.sendMessage(ChatColor.RED
													        + "You are already in the Award Creator. Type cancel to quit or current to see your current step.");
												}
											} else {
												sender.sendMessage(ChatColor.YELLOW
												        + "[VoteRoulette] "
												        + ChatColor.AQUA
												        + "Entered the Award Creator!");
												AwardCreator ac = new AwardCreator(
												        player);
												VoteRoulette.inAwardCreator
												        .put(player, ac);
							...
							if (sender
								...
								if (args.length >= 2) {
									...
									try {
										...
										Reward reward = rm.getRewards().get(
										        rewardIndex);
										if (VoteRoulette.inAwardCreator
										        .containsKey(player)) {
											sender.sendMessage(ChatColor.RED
										...
										VoteRoulette.inAwardCreator.put(player,
										        ac);
										...
										sender.sendMessage(ChatColor.YELLOW
										        ...
										        + "Entered the Award Creator!");
										sender.sendMessage(ChatColor.GRAY
										        + "(You are editting a previous reward)");
									...
									sender.sendMessage(ChatColor.RED
									        + "[VoteRoulette] /vr editreward [#]. Get # from \"/vr rewards -a\"");
								}
							} else {
								sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
							}
						...
						if (args[0].equalsIgnoreCase("deletereward")) {
							if (sender
							        .hasPermission("voteroulette.deleteawards")) {
								if (args.length >= 2) {
									int rewardIndex;
									try {
										rewardIndex = Integer.parseInt(args[1]) - 1;
										...
										sender.sendMessage(ChatColor.AQUA
										        + "[VoteRoulette] You successfully deleted the Reward "
										        + ChatColor.YELLOW
										        + reward.getName()
										        + ChatColor.YELLOW + "!");
									} catch (Exception e) {
									...
									sender.sendMessage(ChatColor.RED
									        + "[VoteRoulette] /vr deletereward [#]. Get # from \"/vr rewards -a\"");
								}
							} else {
								sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
							}
								...
								if (args.length >= 2) {
									...
									try {
										...
										sender.sendMessage(ChatColor.YELLOW
										        ...
										        + "Entered the Award Creator!");
										sender.sendMessage(ChatColor.GRAY
										        + "(You are editting a previous milestone)");
										ac.goToStage(AwardCreationStage.CHOOSE_PRIZE);
									...
									sender.sendMessage(ChatColor.RED
									        + "[VoteRoulette] /vr editmilestone [#]. Get # from \"/vr rewards -a\"");
								}
							} else {
								sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
							}
						...
						if (args[0].equalsIgnoreCase("deletemilestone")) {
							if (sender
								...
								if (args.length >= 2) {
									...
									try {
										...
										rm.getMilestones().remove(
										        milestoneIndex);
										sender.sendMessage(ChatColor.AQUA
										        + "[VoteRoulette] You successfully deleted the Milestone "
										        + ChatColor.YELLOW
										        + milestone.getName()
										        + ChatColor.YELLOW + "!");
									} catch (Exception e) {
									...
									sender.sendMessage(ChatColor.RED
									        + "[VoteRoulette] /vr deletemilestone [#]. Get # from \"/vr milestones -a\"");
								}
							} else {
								sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
							}
							...
							if (args[0].equalsIgnoreCase(plugin.TOP_DEF + "10")
							        || args[0].equalsIgnoreCase(plugin.TOP_DEF)) {
								...
								if (args.length == 1) {
									...
									sender.sendMessage(ChatColor.GOLD
									        ...
									        + plugin.TOP_DEF
									        + " "
									        + plugin.STREAK_DEF.toLowerCase()
									                .replace(" ", ""));
								} else {
									if (args[1]
									        .equalsIgnoreCase(plugin.VOTE_STREAK_DEF
									                .replace(" ", ""))
									        || args[1]
									                .equalsIgnoreCase("votestreaks")
									        || args[1]
									                .equalsIgnoreCase(plugin.STREAK_DEF)) {
										sender.sendMessage(plugin.TOP_10_CMD
										        .replace(
										                "%stat%",
										                plugin.LONGEST_VOTE_STREAK_DEF));
										if (plugin.USE_SCOREBOARD
										        && sender instanceof Player) {
											Utils.showTopScoreboard(
											        (Player) sender,
										...
										if (args[1]
										        .equalsIgnoreCase(plugin.TOTAL_DEF
										                .replace(" ", ""))
										        || args[1]
								...
								if (args[0].equalsIgnoreCase("?")
									...
									Paginate helpMenuPag = new Paginate(
									        Utils.helpMenu(sender),
									        "Help Menu", commandLabel + " "
									                + args[0]);
									if (args.length >= 2) {
										try {
											int pageNumber = Integer
											        .parseInt(args[1]);
											if (pageNumber <= helpMenuPag
											        .pageTotal()) {
												helpMenuPag.sendPage(
												        pageNumber, sender);
											} else {
												sender.sendMessage(plugin.INVALID_NUMBER_NOTIFICATION);
											}
									...
									if (args[0]
									        .equalsIgnoreCase(plugin.LASTVOTE_DEF)) {
										if (args.length == 1) {
											if (!(sender instanceof Player)) {
											...
											if (!sender
											        .hasPermission("voteroulette.lastvote")) {
												sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
												return true;
											}
											if (voter.hasLastVoteTimeStamp()) {
												String timeSince = Utils
												        .getTimeSinceString(voter
											...
											if (args.length == 2) {
												...
												Voter otherVoter = vm
												        .getVoter(otherPlayer);
												if (otherVoter
												        .hasLastVoteTimeStamp()) {
													String timeSince = Utils
										...
										if (args[0]
										        .equalsIgnoreCase(plugin.FORCEVOTE_DEF)) {
											if (!sender
											        .hasPermission("voteroulette.forcevote")) {
												sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
												return true;
											}
											if (args.length == 1) {
											...
											if (args.length > 1) {
												...
												sender.sendMessage(ChatColor.AQUA
												        + "[VoteRoulette] You forced a vote to "
												        + ChatColor.YELLOW
												        + otherPlayer
												        + ChatColor.AQUA + ".");
												...
												forceVote
												        .setServiceName("forcevote");
												if (args.length > 2) {
													forceVote
													        .setServiceName(args[2]);
												}
												Date date = new Date();
												forceVote
												        .setTimeStamp(String.valueOf(date
												                .getTime()));
												forceVote
												        .setUsername(otherPlayer);
											...
											if (args[0]
											        .equalsIgnoreCase(plugin.FORCEREWARD_DEF
											                .toLowerCase())) {
												if (!sender
												        .hasPermission("voteroulette.forceawards")) {
													sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
													return true;
												}
												if (args.length < 3) {
												...
												if (args.length >= 3) {
													...
													ArrayList<Reward> rewards = rm
													        .getRewards();
													if (rewardIndex >= rewards
													        .size()
													        || rewardIndex < 0) {
														sender.sendMessage(plugin.INVALID_NUMBER_NOTIFICATION);
														return true;
													...
													Reward reward = rewards
													        .get(rewardIndex);
													sender.sendMessage(ChatColor.AQUA
													        + "[VoteRoulette] You forced the reward "
													        + ChatColor.YELLOW
													        + reward.getName()
													        + ChatColor.AQUA
													        + " to "
												...
												if (args[0]
												        .equalsIgnoreCase("forcemilestone")) {
													if (!sender
													        .hasPermission("voteroulette.forceawards")) {
														sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
														return true;
													...
													if (args.length >= 3) {
														...
														ArrayList<Milestone> milestones = rm
														        .getMilestones();
														if (milestoneIndex >= milestones
														        .size()
														        || milestoneIndex < 0) {
															sender.sendMessage(plugin.INVALID_NUMBER_NOTIFICATION);
															return true;
														...
														Milestone milestone = milestones
														        .get(milestoneIndex);
														sender.sendMessage(ChatColor.AQUA
														        + "[VoteRoulette] You forced the milestone "
														        + ChatColor.YELLOW
														        + milestone
														                .getName()
														        + ChatColor.AQUA
													...
													if (args[0]
													        .equalsIgnoreCase(plugin.REMIND_DEF)
													        || args[0]
													                .equalsIgnoreCase("reminder")
													        || args[0]
													                .equalsIgnoreCase("broadcast")) {
														if (!sender
														        .hasPermission("voteroulette.remind")) {
															sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
															return true;
														}
														...
														if (args[0]
														        .equalsIgnoreCase(plugin.STATS_DEF)) {
															...
															if (args.length == 1) {
																...
																if (!sender
																        .hasPermission("voteroulette.viewstats")) {
																	sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
																	return true;
																}
																...
																if (plugin.REWARDS_ON_THRESHOLD) {
																	...
																	message = message
																	        + ChatColor.YELLOW
																	        + "\n"
																	        + plugin.VOTE_CYCLE_DEF
																	        + ": "
																	        + ChatColor.AQUA
																	        + voteCycle;
																}
																voteStreak = voter
																        .getCurrentVoteStreak();
																message = message
																        + ChatColor.YELLOW
																        + "\n"
																        + plugin.CURRENT_VOTE_STREAK_DEF
																        + ": "
																        + ChatColor.AQUA
																        + voteStreak;

																longestVoteStreak = voter
																        .getLongestVoteStreak();
																message = message
																        + ChatColor.YELLOW
																        + "\n"
																        + plugin.LONGEST_VOTE_STREAK_DEF
																...
																if (args.length == 2) {
																	...
																	if (args.length == 4) {
																		...
																		// ##
																		if (!sender
																		        .hasPermission("voteroulette.editstats")) {
																			sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
																			return true;
																		}
																		...
																		if (args[2]
																		        .equalsIgnoreCase(plugin.SETTOTAL_DEF)) {
																			otherVoter
																			        .setLifetimeVotes(voteNumber);
																			sender.sendMessage(ChatColor.AQUA
																			        + "[VoteRoulette] You set "
																			        + ChatColor.YELLOW
																			        + otherPlayer
																			        + "'s "
																			        + ChatColor.AQUA
																			...
																			if (args[2]
																				...
																				otherVoter
																				        .setCurrentVoteCycle(voteNumber);
																				sender.sendMessage(ChatColor.AQUA
																				        + "[VoteRoulette] You set "
																				        + ChatColor.YELLOW
																				        + otherPlayer
																				...
																				if (args[2]
																					...
																					otherVoter
																					        .setCurrentVoteStreak(voteNumber);
																					sender.sendMessage(ChatColor.AQUA
																					        + "[VoteRoulette] You set "
																					        + ChatColor.YELLOW
																					        + otherPlayer
														...
														// votes in threshold
															...
															if (args[0]
																...
																if (!sender
																        .hasPermission("voteroulette.viewmilestones")) {
																	sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
																	return true;
																}

																Milestone[] milestones = rm
																        .getQualifiedMilestones(sender
																                .getName());

																if (!(sender instanceof Player)) {
																	milestones = new Milestone[rm
																	        .getMilestones()
																	        .size()];
																	rm.getMilestones()
																	        .toArray(
																...
																if (args.length > 1) {
																	if (args[args.length - 1]
																		...
																		if (sender
																			...
																			ArrayList<Milestone> ms = rm
																			        .getMilestones();
																			milestones = new Milestone[ms
																			        .size()];
																...
																String message = "";
																if (!plugin.GUI_FOR_AWARDS
																        || !(sender instanceof Player)) {
																	for (int i = 0; i < milestones.length; i++) {
																		message += ChatColor.YELLOW
																		        + plugin.MILESTONE_DEF
																		        + " #"
																		        + Integer
																		                .toString(i + 1)
																		        + ": "
																		...
																		if (milestones[i]
																		        .hasItems()) {
																			message += ChatColor.GOLD
																			        + plugin.ITEM_PLURAL_DEF
																			        + ": "
																			        + ChatColor.DARK_AQUA
																			        + Utils.getItemListString(milestones[i]
																			                .getItems())
																			        + "\n";
																		}
																		if (milestones[i]
																			...
																			message += ChatColor.GOLD
																			        ...
																			        + ": "
																			        + ChatColor.DARK_AQUA
																			        + Utils.worldsString(milestones[i]
																			                .getWorlds())
																			        + "\n";
																		}
																		if (milestones[i]
																...
																Paginate milestonePag = new Paginate(
																        message,
																        plugin.MILESTONE_PURAL_DEF,
																        commandLabel
																                + " "
																...
																if (args[0]
																        .equalsIgnoreCase(plugin.REWARDS_PURAL_DEF)) {

																	if (!sender
																	        .hasPermission("voteroulette.viewrewards")) {
																		sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
																		return true;
																	}
																	...
																	if (args.length > 1) {
																		if (args[args.length - 1]
																			...
																			if (sender
																			        .hasPermission("voteroulette.viewallawards")) {
																				ArrayList<Reward> rs = rm
																				        .getRewards();
																				rewards = new Reward[rs
																				        .size()];
																				rs.toArray(rewards);
																				seeingAll = true;
																			}
																		...
																		for (int i = 0; i < rewards.length; i++) {
																			...
																			if (rewards[i]
																				...
																				message += ChatColor.GOLD
																				        ...
																				        + ": "
																				        + ChatColor.DARK_AQUA
																				        + Utils.getItemListString(rewards[i]
																				                .getItems())
																				        + "\n";
																			}
																			if (rewards[i]
																	...
																	Paginate rewardPag = new Paginate(
																	        message,
																	        plugin.REWARDS_PURAL_DEF,
																	        commandLabel
																	                + " "
																	...
																	if (args[0]
																	        .equalsIgnoreCase(plugin.RELOAD_DEF)) {
																		if (!sender
																		        .hasPermission("voteroulette.admin")) {
																			sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
																			return true;
																		}
																		plugin.loadAllFilesAndData();
																		sender.sendMessage(ChatColor.AQUA
																		        + "[VoteRoulette] Reload complete!");
																		// reload
																		// configs
																	} else
																		if (args[0]
																		        .equalsIgnoreCase(plugin.WIPESTATS_DEF)) {
																			if (!sender
																			        .hasPermission("voteroulette.wipestats")) {
																				sender.sendMessage(plugin.NO_PERM_NOTIFICATION);
																				return true;
																			}
																			if (args.length == 1) {
																				sender.sendMessage(ChatColor.RED
																				        + "[VoteRoulette] Please include a players name to wipe or type \"all\" to wipe everyones.");
																				return true;
																			}
																			if (args.length >= 2) {
																				Stat stat;
																				if (args.length == 2) {
																					...
																					stat = Utils
																					        .getVoteStatFromStr(args[2]);
																				}
																				if (stat == null) {
																					sender.sendMessage(ChatColor.RED
																					        + "[VoteRoulette] Unrecognized stat type! Available stats: votecycle, votestreak, longestvotestreak, totalvotes, lastvote, unclaimedrewards, unclaimedmilestones, all");
																					return true;
																				}
																				if (args[1]
																					...
																					sender.sendMessage(ChatColor.AQUA
																					        + "[VoteRoulette] Wiped everyones stat: "
																					        + stat.toString()
																					                .toLowerCase()
																					                .replace(
																					                        "_",
																						...
																						if (args[1]
																							...
																							if (args.length == 2) {
																								...
																								for (Reward reward : unclaimedRewards) {
																									rewardMessages[count] = ChatColor.YELLOW
																									        + Integer
																									                .toString(count + 1)
																									        + ") "
																									        + ChatColor.GOLD
																								...
																								if (args.length == 3) {
																									if (args[2]
																									        .equalsIgnoreCase(plugin.ALL_DEF)) {
																										if (sender
																							...
																							if (args[1]
																								...
																								if (args.length == 2) {
																									...
																									for (Milestone milestone : unclaimedMilestones) {
																										milestoneMessages[count] = ChatColor.YELLOW
																										        + Integer
																										                .toString(count + 1)
																										        + ") "
																										        + ChatColor.GOLD
	...
	void showClickableAwardList(CommandSender sender, AwardType type,
		...
		sender.sendMessage(ChatColor.GOLD + "-----" + ChatColor.GREEN + typeStr
		        + ChatColor.GOLD + " | " + ChatColor.GREEN + "Page "
		        + pageNumber + "/" + totalPages + ChatColor.GOLD + "-----");
		if (pageNumber == 1) {
			sender.sendMessage(ChatColor.GOLD
			        + "Click on a name to see it's contents.");
		}
		...
		while (count < 7 * pageNumber && count < awards.length) {
			...
			String command = "/" + plugin.DEFAULT_ALIAS + " "
			        + typeStr.toLowerCase() + " see "
		...
		if (pageNumber < totalPages) {
			sender.sendMessage(ChatColor.GOLD + "Type " + "\"/"
			        + plugin.DEFAULT_ALIAS + " " + typeStr.toLowerCase() + " "
