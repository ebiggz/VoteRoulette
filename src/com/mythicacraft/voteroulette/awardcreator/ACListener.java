	...
	private void processChat(Player player, String message) {
		if(VoteRoulette.inAwardCreator.containsKey(player)) {
			...
			if(message.equalsIgnoreCase("preview")) {
				...
				Utils.showAwardGUI(ac.getAward(), VoteRoulette.getVoterManager().getVoter(player.getName()), 1, true);
				return;
			}
			...
			switch(stage) {
				...
				case CHOOSE_OPTION:
					...
					else if(message.equalsIgnoreCase("websites") && ac.getAwardType() == AwardType.REWARD) {
						ac.goToStage(AwardCreationStage.ADD_WEBSITES);
					}
					else if(message.equalsIgnoreCase("votestreak") && ac.getAwardType() == AwardType.REWARD) {
						ac.goToStage(AwardCreationStage.ADD_VOTESTREAK);
					}
					else if(message.equalsIgnoreCase("priority") && ac.getAwardType() == AwardType.MILESTONE) {
						ac.goToStage(AwardCreationStage.SET_PRIORITY);
					}
					else if(message.equalsIgnoreCase("recurring") && ac.getAwardType() == AwardType.MILESTONE) {
						ac.goToStage(AwardCreationStage.SET_RECURRING);
					}
					else if(message.equalsIgnoreCase("votes") && ac.getAwardType() == AwardType.MILESTONE) {
					...
					if(message.equalsIgnoreCase("finish") || message.equalsIgnoreCase("save") || message.equalsIgnoreCase("done")) {
						if(ac.hasOrigAward()) {
							...
							if(a.getAwardType() == AwardType.REWARD) {
								VoteRoulette.getAwardManager().getRewards().remove(ac.getOrigIndex());
							} else {
								VoteRoulette.getAwardManager().getMilestones().remove(ac.getOrigIndex());
							}
						}
