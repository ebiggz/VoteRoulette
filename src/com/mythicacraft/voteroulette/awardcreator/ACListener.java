...
public class ACListener implements Listener {
	...
	private void processChat(Player player, String message) {
		if (VoteRoulette.inAwardCreator.containsKey(player)) {
			...
			if (message.equalsIgnoreCase("cancel") || message
			        .equalsIgnoreCase("quit") || message
			        .equalsIgnoreCase("stop") || message
			        .equalsIgnoreCase("exit")) {
				VoteRoulette.inAwardCreator.remove(player);
				player.sendMessage(ChatColor.YELLOW + "* " + message
				        .replaceFirst(Character.toString(message.charAt(0)),
				                Character.toString(message.charAt(0))
				                        .toUpperCase()) + "ing award creation...");
				player.sendMessage(ChatColor.YELLOW + "[VoteRoulette] " + ChatColor.AQUA + "Exited the Award Creator!");
				return;
			}

			if (message.equalsIgnoreCase("help") || message
			        .equalsIgnoreCase("?")) {
			...
			if (message.equalsIgnoreCase("current") || message
			        .equalsIgnoreCase("step")) {
			...
			switch (stage) {
				...
				case CHOOSE_OPTION:
					...
					} else if (message.equalsIgnoreCase("websites") && (ac
					        .getAwardType() == AwardType.REWARD)) {
						ac.goToStage(AwardCreationStage.ADD_WEBSITES);
					} else if (message.equalsIgnoreCase("votestreak") && (ac
					        .getAwardType() == AwardType.REWARD)) {
						ac.goToStage(AwardCreationStage.ADD_VOTESTREAK);
					} else if (message.equalsIgnoreCase("priority") && (ac
					        .getAwardType() == AwardType.MILESTONE)) {
						ac.goToStage(AwardCreationStage.SET_PRIORITY);
					} else if (message.equalsIgnoreCase("recurring") && (ac
					        .getAwardType() == AwardType.MILESTONE)) {
						ac.goToStage(AwardCreationStage.SET_RECURRING);
					} else if (message.equalsIgnoreCase("votes") && (ac
					        .getAwardType() == AwardType.MILESTONE)) {
				...
				case CHOOSE_PRIZE:
					if (ac.getAward() != null) {
						if (!ac.getAward().isEmpty()) {
							if (message.equalsIgnoreCase("next") || message
				...
				case FINAL_CONFIRMATION:
					if (message.equalsIgnoreCase("finish") || message
					        .equalsIgnoreCase("save") || message
					        .equalsIgnoreCase("done")) {
						if (ac.hasOrigAward()) {
							Award a = ac.getOrigAward();
							VoteRoulette.getAwardManager().deleteAwardFromFile(
							        a);
						...
						player.sendMessage(ChatColor.YELLOW + "* Successfully created and saved the " + ac
						        .getAwardType().toString().toLowerCase() + " " + ChatColor.AQUA + ac
						        .getAward().getName() + ChatColor.YELLOW + "!");
						VoteRoulette.inAwardCreator.remove(player);
						player.sendMessage(ChatColor.YELLOW + "[VoteRoulette] " + ChatColor.AQUA + "Exited the Award Creator!");
					}
					break;
				case NAME:
					Award award = VoteRoulette.getAwardManager()
					        .getAwardByName(message, awardType);
					if (award != null) {
						player.sendMessage(ChatColor.RED + "There is already a " + awardType
						        .toString().toLowerCase() + " with this name!");
